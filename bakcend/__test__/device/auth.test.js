...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Amari.Stoltenberg',
        'password':'0Qg9UW9ay3urJqk',
        'email':'Catharine.Fay60@gmail.com',
        'name':'Laura Schulist',
        'userType':authConstant.USER_TYPES.User,
        'phone_number':'generating',
        'profile_img':'policy',
        'sign_img':'reboot',
        'aadhar_img':'hacking',
        'pan_img':'Clothing',
        ...
        'blob':'Malawi',
        'age':95,
        'profession':'multi-byte',
        'experience':'Games',
        'income':'Representative',
        'dob':'2023-05-24T00:20:52.280Z',
        'isAuth':false,
        'mobileNo':'(211) 347-8498'
      });
...
describe('POST /login -> if username and password is correct', () => {
  test('should return user with authentication token', async () => {
    let user = await request(app)
      ...
      .send(
        {
          username: 'Catharine.Fay60@gmail.com',
          password: '0Qg9UW9ay3urJqk'
        }
      );
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    let user = await request(app)
      .post('/device/auth/forgot-password')
      .send({ 'email':'Catharine.Fay60@gmail.com', });
